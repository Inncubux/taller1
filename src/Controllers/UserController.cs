...
namespace ECommerce.src.Controllers
{
    public class UserController(ILogger<UserController> logger, UnitOfWork unitOfWork) : BaseApiController
    {
        ...
        /*
                ...
                {
                    var query =  _unitOfWork.UserRepository.GetUserQueryable();

                    if(userParams.Status.HasValue)
                    {
                        query = query.Where(x => x.UserName == userParams.Status.Value);
                    }

                    if(!string.IsNullOrWhiteSpace(userParams.SearchTerm))
                    {
                        var term = userParams.SearchTerm.ToLower();
                        query = query.Where(x => x.UserName.Contains(term, StringComparison.CurrentCultureIgnoreCase) ||
                    ...
                    var userDtos = users.Select(x => new UserDto.UserToUserDto(x)).ToList();
