    ...
    namespace ECommerce.src.Controllers
    {
        ...
        public class UserController(ILogger<UserController> logger, UnitOfWork unitOfWork) : BaseApiController
        {
            ...
            public async Task<ActionResult<ApiResponse<string>>> ChangePassword([FromBody] ChangePasswordDto changePasswordDto)
            {
                ...
                // Validación de propiedad del recurso basada en JWT (el email proviene del token)
                var isAdmin = User.IsInRole("Admin");
                if (!isAdmin && user.Email != userEmail)
                {
                    return Forbid(); // No tiene derecho a cambiar la contraseña de otro
                }

                if (string.IsNullOrWhiteSpace(changePasswordDto.Password))
                {
                    return BadRequest(new ApiResponse<string>(false, "La contraseña actual no puede estar vacía."));
                }
